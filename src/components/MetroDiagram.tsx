...
export default function MetroDiagram() {
  ...
  const generateLinePath = (stations: string[]) => {
    ...
    stations.forEach((stationId, index) => {
      const station = metroStations[stationId];
      if (!station) return;
      if (index === 0) {
        d += `M ${station.x} ${station.y} `;
      } else {
        d += `L ${station.x} ${station.y} `;
      }
    });
    return d;
  };
    ...
    >
      ...
      <div className="absolute top-4 left-4 z-20 bg-white/90 dark:bg-[#242434]/90 backdrop-blur-md p-4 rounded-xl border border-[#E5E7EB] dark:border-[#3E3E4E] shadow-lg pointer-events-none">
        <h3 className="text-sm font-bold text-[#111118] dark:text-[#F1F1F4] mb-3 flex items-center gap-2">
          <Train size={18} className="text-[#FF6B35]" /> Metro Lines
        </h3>
      ...
      <div ref={containerRef} className="flex-1 overflow-auto custom-scrollbar relative">
        <div 
          ...
          style={{ width: 3000, height: 3000, transform: `scale(${scale})` }}
        >
          {/* Static SVG for lines to prevent rendering bugs */}
          <svg width="3000" height="3000" className="absolute inset-0 pointer-events-none">
            {/* Draw thick lines */}
            {metroLines.map(line => (
              <path
                key={line.id}
            ...
            {Object.values(metroStations).map(node => node.isInterchange ? (
              <circle
                key={`bg-${node.id}`}
                cx={node.x}
                cy={node.y}
              ...
              <line
                key={`bg-${node.id}`}
                x1={node.orientation === 'right' || node.orientation === 'left' ? node.x - 8 : node.x}
                y1={node.orientation === 'right' || node.orientation === 'left' ? node.y : node.y - 8}
                x2={node.orientation === 'right' || node.orientation === 'left' ? node.x + 8 : node.x}
                y2={node.orientation === 'right' || node.orientation === 'left' ? node.y : node.y + 8}
                stroke={metroLines.find(l => l.id === node.lines[0])?.color || '#FFFFFF'}
                strokeWidth={6}
                strokeLinecap="round"
                transform={node.orientation.includes('angled') ? `rotate(45 ${node.x} ${node.y})` : ''}
              />
            ))}
          </svg>

          {/* HTML Overlay for Labels & Interactions */}
          {Object.values(metroStations).map(node => (
            <div
              key={node.id}
